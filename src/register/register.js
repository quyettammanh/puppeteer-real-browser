...
const {
  cancellBooking,
  getActiveStepText,
} = require("./helper/click_cancel_booking.js");
const { acceptCookies } = require("./helper/click_cookies.js");
  ...
  while (attempts < maxAttempts && Date.now() - startTime < timeout) {
    try {
      ...
      if (handler && !stepCompletionStatus[currentStep]) {
        ...
        // Thêm biến đếm cho trường hợp unknown step
        if (!stepAttempts['unknown']) {
          stepAttempts['unknown'] = 0;
        }
        
        // Tăng biến đếm
        stepAttempts['unknown']++;
        ...
        log(`Unknown step attempt ${stepAttempts['unknown']}/3`);
        await randomTime(3, 4);
      }
    } catch (error) {
      attempts++;
